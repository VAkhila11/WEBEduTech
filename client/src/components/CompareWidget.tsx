...
export default function CompareWidget() {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-4 left-4 right-4 md:bottom-8 md:right-8 md:left-auto z-40 bg-white dark:bg-dark-card border border-slate-200 dark:border-dark-border rounded-2xl shadow-2xl p-4 max-w-sm w-full transition-all duration-300 transform scale-100 hover:shadow-brand-indigo/10"> {/* responsive bottom bar positioning */}
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-slate-100 dark:border-slate-800"> {/* strict mb-4 (16px), pb-2 (8px) */}
          <div className="flex items-center space-x-2 text-brand-indigo dark:text-indigo-400 font-semibold text-sm"> {/* strict space-x-2 (8px) */}
        ...
        <div className="space-y-2 mb-4"> {/* strict space-y-2 (8px), mb-4 (16px) */}
        ...
        <button
          ...
          className={`w-full py-2.5 px-4 rounded-xl text-sm font-semibold text-white flex items-center justify-center space-x-2 transition-all ${
        ...
        {compareColleges.length < 2 && (
          <p className="text-[10px] text-slate-400 mt-2 flex items-center space-x-1"> {/* strict mt-2 (8px) */}

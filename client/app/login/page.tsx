...
import { Sparkles, Mail, Lock, Phone, KeyRound, Globe, ArrowRight, ShieldCheck } from "lucide-react";
    ...
    <div className="flex-grow flex items-center justify-center py-16 px-4 relative">
      ...
      <div className="glass-card max-w-md w-full p-8 rounded-3xl shadow-2xl relative border border-slate-200/50 dark:border-slate-800">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Discover your path at EDUTech</p>
        </div>
        ...
        {loginMode === "password" && (
          <form onSubmit={handlePasswordLogin} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-gradient-to-r from-brand-indigo to-brand-blue text-white rounded-xl font-bold text-sm hover:shadow-lg shadow-brand-indigo/15 hover:-translate-y-0.5 transition-all flex items-center justify-center space-x-1.5"
            >
            ...
            {!otpSent ? (
              <div className="space-y-4">
                ...
                <button
                  ...
                  disabled={loading}
                  className="w-full py-3.5 bg-gradient-to-r from-brand-indigo to-brand-blue text-white rounded-xl font-bold text-sm hover:shadow-lg transition-all"
                >
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-4 animate-in fade-in duration-200">
                {mockOtpHint && (
                  <div className="p-3 bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-400 rounded-xl text-xs flex items-center space-x-2 font-semibold">
                ...
                <div>
                  ...
                  <div className="relative flex items-center bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-xl focus-within:ring-2 focus-within:ring-brand-indigo transition-all">
                    <KeyRound className="h-4.5 w-4.5 text-slate-400 ml-3.5 absolute pointer-events-none" />
                    <input
                      type="text"
                      placeholder="123456"
                      value={otpCode}
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-3 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-350 rounded-xl text-sm font-semibold flex items-center justify-center space-x-2 transition-all"
        >
        ...
        <p className="text-center text-xs text-slate-400 mt-6">
          ...
          <Link href="/signup" className="text-brand-indigo hover:underline font-semibold">
            Create Account
          </Link>
        </p>
      </div>
